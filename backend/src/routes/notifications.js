...
r.get('/', authenticate, async (req, res) => {
  try {
    ...
    const [notifications, unread, total] = await Promise.all([
      db.query(sql, params),
      db.queryOne('SELECT COUNT(*) AS c FROM notifications WHERE user_id=? AND is_read=0', [req.userId]),
...
r.get('/count', authenticate, async (req, res) => {
  const row = await db.queryOne('SELECT COUNT(*) AS c FROM notifications WHERE user_id=? AND is_read=0', [req.userId]);
...
r.post('/push-token', authenticate, async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO push_tokens (user_id, token, platform) VALUES (?,?,?) ON DUPLICATE KEY UPDATE token=?, updated_at=NOW()', [req.userId, token, platform, token]).catch(() => {});
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
