...
class _PS extends ConsumerState<PaymentScreen> {
  ...
  void _reset() {
    setState(() {
      _err = null; _status = null; _needsOtp = false;
      _waitingReturn = false; _orderId = null;
      _paypalOrderId = null; _flwRef = null;
      _otpCtrl.clear();
    });
  }
  ...
  Future<void> _pay() async {
    if (_method.isMobileMoney && _phoneCtrl.text.trim().isEmpty) {
      _setErr('Enter your phone number');
      return;
    }
  ...
  Future<void> _doStripe() async {
    ...
    if (confirm.data['success'] == true) {
      _showSuccess(txId: confirm.data['order_id'] as String? ?? _orderId ?? '', method: 'Stripe Card', details: confirm.data);
    } else {
  ...
  Future<void> _doPayPal() async {
    ...
    final result = await Navigator.push<bool>(context, MaterialPageRoute(builder: (_) => PaymentWebViewScreen(
      url: approveUrl, gateway: 'paypal',
      orderId: _orderId, paypalOrderId: _paypalOrderId,
    )));
  ...
  Future<void> _capturePayPal() async {
    ...
    if (r.data['success'] == true) {
      _showSuccess(txId: r.data['order_id'] as String? ?? _orderId ?? '', method: 'PayPal', details: r.data);
    } else {
  ...
  Future<void> _doFlutterwave() async {
    ...
    if (r.data['requires_otp'] == true || r.data['auth_mode'] == 'otp') {
      ...
      if (ok == true && mounted) {
        ...
        setState(() { _waitingReturn = true; _status = 'Return here after completing payment in browser'; });
      }
    } else {
  ...
  Future<void> _validateOtp() async {
    ...
    if (r.data['success'] == true) {
      _showSuccess(txId: r.data['order_id'] as String? ?? _orderId ?? '', method: _method.label, details: r.data);
    } else {
  ...
  Future<void> _pollFlutterwave() async {
    ...
    if (mounted) {
      setState(() { _busy = false; _waitingReturn = true; _status = "If you approved on your phone, tap \"I've Paid\" below"; });
    }
  }
    ...
    showDialog(
      ...
      builder: (_) => Dialog(
        ...
        child: Padding(padding: const EdgeInsets.all(28), child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 80, height: 80, decoration: const BoxDecoration(color: Colors.green, shape: BoxShape.circle),
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 8),
          ...[PayMethod.mtnRw, PayMethod.airtelRw, PayMethod.mpesaKe].map((m) => Padding(
            padding: const EdgeInsets.only(bottom: 8),
            child: _MethodCard(method: m, selected: _method == m, onTap: () => setState(() { _method = m; _reset(); }), subtitle: m.label, dark: dark),
          )),
          if (_method.isMobileMoney) ...[
            ...
            TextField(
              ...
              decoration: InputDecoration(hintText: 'Phone Number', prefixIcon: Icon(Icons.phone_rounded, color: AppTheme.orange)),
            ),
          ],
          if (_needsOtp) ...[
            const SizedBox(height: 12),
            TextField(controller: _otpCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'OTP / PIN')),
            const SizedBox(height: 8),
          ...
          const SizedBox(height: 20),
          if (!_needsOtp && !_waitingReturn) SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: _busy
                ...
                : Text('Pay \$${widget.priceUsd.toStringAsFixed(2)}'),
            ),
          ),
          if (_waitingReturn) ...[
            const SizedBox(height: 12),
            ElevatedButton(
              onPressed: _method.isPayPal ? _capturePayPal : _verifyManually,
          ...
          const Center(child: Text(
            'Payments processed by Stripe, PayPal & Flutterwave.\nNo card data is stored on our servers.',
    ...
    child: AnimatedContainer(
      ...
      child: Row(children: [
        Container(
          ...
          child: Center(child: Text(method.emoji, style: const TextStyle(fontSize: 18))),
        ),
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (badges.isNotEmpty) ...[
            ...
            Wrap(spacing: 4, children: badges.map((b) => Container(
              ...
              decoration: BoxDecoration(color: method.color.withOpacity(0.1), borderRadius: BorderRadius.circular(4)),
              child: Text(b, style: TextStyle(fontSize: 10, color: method.color, fontWeight: FontWeight.w600)),
            )).toList()),
          ],
        ])),
